...
**Environment roles (RBAC v2):** Workato allows exactly one environment role per collaborator per environment. Granting an environment role through ConductorOne replaces the collaborator's existing role in that environment — ConductorOne models the roles as mutually exclusive entitlements, so the previous role's grant is removed automatically. Directly revoking an environment role is not supported by the connector; to change a collaborator's access, grant the replacement role instead.
